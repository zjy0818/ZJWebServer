...
void *epoll_init(struct event_loop *eventLoop) 
{
    ...
    if (epollDispatcherData->efd == -1) {
        printf("epoll_ctl create failed\n");
    }
...
//    event.events = events | EPOLLET;
	...
	{
        printf("epoll_ctl delete fd failed\n");
    }
...
//    event.events = events | EPOLLET;
	...
	{
        printf("epoll_ctl modify fd failed\n");
    }
...
int epoll_dispatch(struct event_loop *eventLoop, struct timeval *timeval) 
{
    ...
    for (i = 0; i < n; i++) 
	{
		...
		{
            printf("get message channel fd==%d for read, %s\n", epollDispatcherData->events[i].data.fd, eventLoop->thread_name);
		...
		{
            printf("get message channel fd==%d for write, %s", epollDispatcherData->events[i].data.fd,eventLoop->thread_name);
