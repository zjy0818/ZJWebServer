...
//套接字接收数据，调用 buffer_socket_read 函数接收来自套接字的数据流，并将其缓冲到 buffer 对象中。
	...
	message_call_back messageCallBack, write_completed_call_back writeCompletedCallBack)
{
	...
	//完成对connectionCompletedCallBack的函数回调
...
int tcp_connection_send_data(struct tcp_connection *tcpConnection, void *data, int size)
{
	...
	//先往套接字尝试发送数据
	// 如果发现当前 channel 没有注册 WRITE 事件，并且当前 tcp_connection 对应的发送缓冲无数据需要发送
	// 就直接调用 write 函数将数据发送出去
	...
	//如果这一次发送不完，就将剩余需要发送的数据拷贝到当前 tcp_connection 对应的发送缓冲区中，
	//并向 event_loop 注册 WRITE 事件，这样数据就由框架接管，应用程序释放这部分数据。
	if (!fault && nleft > 0)
	{
